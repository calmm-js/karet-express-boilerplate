...
export const transitionInOut = ({wrapperClass, transitionDuration, delayDuration}) =>
  U.pipe(U.foldPast((o, next) => ({next, prev: o.next}), {next: null}),
         U.flatMapSerial(({prev, next}) => {
           ...
           const actions = []
           if (prev)
             actions.push(wrap(prev, false),
                          U.later(transitionDuration, null))
           if (next)
             actions.push(wrap(next, false),
...
export const transitionMaxHeight = ({content, show, transitionPeriod}) => {
  ...
  return U.parallel([
    ...
    U.seq(show,
          U.changes,
          U.flatMapLatest(s => {
            const maxHeight = `${content.get() && content.get().getBoundingClientRect().height}px`
            return U.serially(
              s ? [{overflowY, maxHeight, transition},
